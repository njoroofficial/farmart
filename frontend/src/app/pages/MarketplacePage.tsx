...
export function MarketplacePage() {
  ...
  const filtered = useMemo(() => {
    let result = animals.filter((a) => a.is_available);
    ...
    if (breed) result = result.filter((a) => a.breed.name === breed);
    if (location) result = result.filter((a) => a.location === location);

    if (minAge) result = result.filter((a) => a.age_months >= Number(minAge));
    if (maxAge) result = result.filter((a) => a.age_months <= Number(maxAge));
      ...
      <div className="bg-[#2D6A4F] text-white py-10">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <p className="text-[#B7E4C7] text-sm">
            Browse {animals.length}+ animals from verified farmers across Kenya
          </p>
